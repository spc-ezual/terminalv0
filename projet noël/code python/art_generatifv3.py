...
from cmath import cos,sinh,sin
...
def creation_mandelbrot_carré(taille,debut_x=-2,debut_y=-1.5,fin_x=1,fin_y=1.5,pressision=500):
    font_plan=Image.new("RGB",taille)
    pressision=ceil(pressision/271)
    for k in range (taille[0]):
        for j in range (taille[1]):
            font_plan.putpixel((k,j),coul(quadratique(complex(0,0),co_point([k,j],taille,debut_x,fin_x,debut_y,fin_y),pressision)))
    font_plan.save("mandelbrot.jpg")
...
def creation_ensemble_cos(taille,debut_x=-2,debut_y=-2,fin_x=2,fin_y=2,pressision=500):
    ...
    for k in range (taille[0]):
        for j in range (taille[1]):
            font_plan.putpixel((k,j),coul(cosin(complex(0,0),co_point([k,j],taille,debut_x,fin_x,debut_y,fin_y),pressision)))
...
def map_noise(taille,echelle,impacte,lacunariter,graine):
...
creation_mandelbrot_carré((700,700))
print(78)